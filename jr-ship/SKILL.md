...
4. **Decide whether to split**. First confirm you actually hold an analysis; only then apply the routing below.

   - **A zero return is not "no split recommended"**: those are distinct outcomes and MUST NOT collapse. A delegated sub-agent that returns nothing, an empty result, or an error produced **no analysis**, which pattern-matches the "splitting is NOT recommended" branch below and would have the lead skip to step 6 and ship the whole diff as one unsplit PR — a recommendation the run never made. Do NOT fall through on a zero return, and do NOT evaluate the routing or the heuristic below: there is nothing to route on. Report `Split-analysis agent UNREPORTED — the diff was not analysed` and ask: `Options: [Proceed single-PR — unanalysed] / [Retry the analysis]`. This check binds under `--split-only` too: a forced split with no analysis has no groups to split into, so the same two options apply, and [Proceed single-PR — unanalysed] overrides the flag. Step 5's confirmation and its `advisor()` do not cover this: both would show a plausible single-PR outcome with nothing to say the analysis was lost. Never present an unsplit ship as the analysis's verdict when the analysis never returned; the spawn rule's `name:` omission (Model requirements, `../shared/subagent-reporting.md`) makes a zero return the rare edge rather than the norm, which is exactly why it must be handled explicitly instead of by absence. **No tree state is at risk**: the split-analysis agent is analysis-only (it is given no code-edit mandate and Phase 2 runs before any branch or commit exists), so unlike the CI-fix agent there is nothing to revert — the only loss is the analysis.

   **If `--split-only` is set, force the multi-PR flow regardless of the heuristic** — proceed to step 5 with all detected groups (the user's explicit override of the split-vs-single decision). Otherwise apply the heuristic below.
...
- If CI fails on any sub-PR and the **CI-failure handling** procedure cannot bring it green (fix skipped, no code-level fix, an UNREPORTED zero return, secret-halt, or the 2-cycle cap reached), report the failure URL and stop — do NOT merge remaining PRs in the chain. List which PRs were merged and which are still open.
...
- If the split analysis returns groups that are nonsensical or otherwise unusable, fall back to the single-PR flow and inform the user. This does NOT cover a zero return (nothing, an empty result, or an error): that is the absence of an analysis rather than a bad one, and Phase 2 step 4 owns it, so route there rather than falling back here.
