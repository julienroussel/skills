...
jr-review/protocols/                  — skill-local procedures read at Phase 1 Track A under hard-fail +
                                     ...
                                     Pattern C, #66), post-fix.md (Phase 5.5 simplification + Phase 5.55
                                     fix-verification bodies; grep-checked at Phase 1 for existence +
...
jr-ship/protocols/                    — skill-local procedures read at Phase 1 under hard-fail + smoke-parse
                                     ...
                                     line-anchored `^#### Phase 3b: Multi-PR Flow` AND `step 12-multi`,
                                     then Read at Phase 3b entry ONLY when a split is confirmed, so
                                     single-PR / --no-split / resume / --dry-run runs never load it)
...
jr-skill-audit/protocols/finding-validation.md — Phase 3 step 1 + step 2 bodies (`file` containment, `codeExcerpt`
                                     sanity-check, the five source-citation branches); read at Phase 1 Track A
                                     UNCONDITIONALLY (hard-fail + smoke-parse, anchors `Contain before opening` AND
                                     `Source-citation validation`), applied at Phase 3. The extraction also replaced
                                     the enumerated allowlist with a `realpath` containment predicate — rationale in
                                     the file.
...
jr-skill-audit/protocols/shadow-detection.md — the `scope-resolution` finding shape emitted when a skill
                                     basename resolves in BOTH personal and project scope (anchor field,
                                     `clarify: true`, `source` as a cache/refs.json key, `scope: personal`
                                     as the runtime winner) plus its design rationale. Loaded **on demand**,
                                     only when a cross-scope collision is detected — reference material, NOT
                                     a Phase 1 Track A hard-fail read (mirrors edge-cases.md).
...
- Both `/jr-audit` and `/jr-review` support `--converge[=N]`: a re-review loop that wraps Phases 2–6 in a repeatable cycle with auto-approval. Both default effort-adaptively from `CLAUDE_EFFORT` (`low`/`medium` → 2, `high` → 3, `xhigh`/`max` → 5); each skill declares its own `effort`, so a session `/effort` change does not move these defaults. `/jr-review` caps at 10 and runs a fresh-eyes security pass after convergence. `/jr-audit` clamps tighter at 5, because the per-iteration blast radius is higher.
