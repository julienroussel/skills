...
shared/claim-verification.md       — canonical anti-hallucination doctrine: code-internal vs external-authority
                                     ...
                                     Track A by /audit, /review, /skill-audit; cross-ref'd by /ship, /tackle,
                                     /doctor. /skill-audit's live refs-cache is the reference Tier-2 impl.
...
- `claim-verification.md` — read at Phase 1 Track A by `/audit`, `/review`, `/skill-audit` and passed to reviewers as context; the lead applies its claim classification + cap/verify at Phase 3 and the no-autonomous-decision-without-a-checked-fact rule at every auto-apply/merge site. Cross-referenced (not Track-A-read) by `/ship`'s CI-fix protocol and `/tackle`'s rigor protocol. `/skill-audit`'s live refs-cache is the reference Tier-2 implementation of the doctrine.
