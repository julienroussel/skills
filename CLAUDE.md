...
- `gitignore-enforcement.md` — the lead agent applies the protocol at each `.claude/*` write site (cache files, audit reports, suppressions, health snapshots). **`/jr-audit` states the command and the per-path reason ONCE**, in its "Cache-write security checks" table (Phase 1), and each call-site carries only `Security check (enforced): cache-write protocol for <path>`; a site with a genuine deviation (e.g. the audit-report glob append) states just that deviation. This replaced six near-verbatim restatements; a single table cannot drift the way six copies can. `/jr-review` still inlines its per-site reasons; converge it the same way if it grows a sixth site. The prose expansion of warn/append behaviour lives in the shared file only.
