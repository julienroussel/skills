...
**Mechanism note.** The idle-notification path is the agent-teams **teammate** mechanism — a core-harness feature since v2.1.178, **not** the `agent-teams` *plugin* the 2026-07-16 run is labelled against (that plugin was replaced by native `.claude/agents/*.md` defs, #72). Agent teams are enabled in this environment (`CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS=1`) and the 2×2 above was measured with them on. [unverified] The run never varied that env var, so whether a named spawn still loses its result with teams **off** (Claude Code's default) is untested — the sub-agents doc contemplates named non-teammate spawns, so assume nothing there. This flags an axis the run did not cross; it does not reopen the measured result (across the tested axes, `name:` is the determinant). The Spawn rule is safe regardless: unnamed always returns, so never naming a work-producing subagent costs nothing.
