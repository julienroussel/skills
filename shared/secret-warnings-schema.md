...
The `consumerEnforcement` value is `"not-implemented"` until `/jr-ship` (or another consumer) implements the block-on-match contract; at that point it flips to `"enforced"`.
...
| `missingRunCount` | integer | absent | Tracks consecutive `/jr-review` runs where the entry's referenced file was missing. Increments in step 3(b); resets to `0` when the file re-appears in step 3(c). After 3 consecutive runs missing, the entry becomes eligible for pruning. |
...
In addition to the allowlist regex above, every `file` value (read or written) MUST pass these checks (mirrors `--scope` checks 4-6 in `/jr-review`'s "Parameter sanitization"):
...
This parallels `[SECRET-WARNINGS BACKUP FAILED]` — validation failure at the audit-trail-write path is a hard failure, never a silent drop, because the alternative is a secret that evades the pre-commit hook and any future `/jr-ship` enforcement.
...
3. On successful backup, emit a Phase 7 ACTION REQUIRED entry: `ACTION REQUIRED: secret-warnings.json failed schema validation and was backed up to <path>. Inspect manually before any future /jr-review run.`
...
Before the first write attempt in a `/jr-review` run, probe for `flock(1)` with a platform-agnostic check:
...
If the execution environment cannot guarantee BOTH atomic-rename AND `flock` (e.g., `FLOCK_AVAILABLE=false`), fall back to per-session filenames as a hard requirement: use `secret-warnings-${baseCommit:0:8}-${CLAUDE_SESSION_ID:-$(date +%s)}.json` as the filename. The `$CLAUDE_SESSION_ID` env var (exposed by Claude Code v2.1.138+) is per-session unique and avoids the timestamp-collision risk of fast back-to-back runs within the same wall-clock second. The `:-$(date +%s)` fallback preserves backwards-compatibility for older Claude Code versions that do not expose the variable. Each `/jr-review` (or `/jr-audit`) run writes its own file, eliminating the concurrent-writer problem. This is a hard fallback, not an optional optimization.

For CI matrix builds sharing a workspace, use the per-session filename strategy by default. Consumers (`/jr-ship`, the pre-commit hook) MUST glob `.claude/secret-warnings*.json` to enumerate all matching files.
