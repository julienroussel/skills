...
4. Update each consumer's Phase 1 Track A read list (in their respective SKILL.md) if the file is required by that consumer. Consumers that don't read the file still benefit — `/jr-doctor` Group D smoke-parses every row in the table.
5. Update `CLAUDE.md` `shared/ — single source of truth` section with a one-line inventory entry and a usage-pattern bullet.
6. Run `/jr-doctor` and confirm Group D picks up the new file with no smoke-parse failure.
...
Example: `/jr-audit` consumes only the `Advisory-tier classification` section of `secret-scan-protocols.md` — it does not need `isHeadless` or `userContinueWithSecret`. Under the union strategy, `/jr-audit` will still hard-abort if either of those substrings disappears from `secret-scan-protocols.md`.
