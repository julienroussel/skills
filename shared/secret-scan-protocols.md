...
`/jr-ship` is a third consumer (inline at its Phase 1, applied at the pre-commit secret halt). It executes behaviors 1, 4 and 5 (ACTION REQUIRED logging, the `⚠ SECRET STILL PRESENT` warning, the non-zero exit latch) and has three documented gaps: **behavior 2** (audit-trail write) is declined by design — `/jr-ship` is a future *reader* of `.claude/secret-warnings.json`, not a writer; **behavior 3** is unavailable for the same structural reason as `/jr-audit` and is likewise reported, not skipped; **behavior 6** (suppression-list snapshot) does not apply, since `/jr-ship` has one scan site and no re-scan loop, so nothing can re-prompt. This note is what licenses that subset — a consumer running a subset without a gap recorded here is out of contract.
