...
- `/jr-review --converge` has an effort-adaptive iteration default and caps at 10. After convergence, a **fresh-eyes security pass** runs with a clean reviewer context to catch regressions introduced by auto-fixes. From iteration 3+ the lead calls `advisor()` to spot compound drift before another iteration fires.
- `/jr-audit --converge` caps at 5, tighter than `/jr-review`, because the per-iteration blast radius is higher (full-codebase scope vs. PR diff). Pre-iteration `advisor()` fires from iteration 2+ for the same drift detection.
