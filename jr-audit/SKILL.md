...
<!-- Dependencies:
  ...
  Skill-local protocol files (resolved via ${CLAUDE_SKILL_DIR}/; read at Phase 1 Track A under hard-fail + smoke-parse guard):
    ...
    - protocols/fix-secret-validate.md          — Phase 5.6 + Phase 6 bodies; grep-checked at Phase 1 (existence + anchors, NOT loaded), Read at Phase 5 entry (deferred / Pattern C)
...
- `--converge[=N]` — Re-audit modified files after Phase 6 (validate-fix loop) until no new findings emerge OR `N` iterations are reached. **Default N depends on `CLAUDE_EFFORT`** (read at runtime via Bash: `effort="$CLAUDE_EFFORT"; [ -z "$effort" ] && effort=high`; when the read comes back empty the fallback fired, so print `Effort: unset — defaulting to high` in the Phase 1 output rather than defaulting silently, since this skill declares `effort: max` and a silent fallback would resolve bare `--converge` to 3 instead of 5): `low`/`medium` → 2, `high` → 3, `xhigh`/`max` → 5 (mirrors `/jr-review`'s effort-adaptive default, bounded by this skill's cap below). This skill declares `effort: max`, so bare `--converge` resolves to 5 on Claude Code ≥ 2.1.133. An explicit `--converge=N` overrides the effort-adaptive default. Capped at `[2, 5]` — values outside the range warn and are clamped (`N=1` is upgraded to 2 with a warning since one iteration provides no re-audit; `N>5` is clamped to 5 to limit runtime). Mutually exclusive with `nofix` (no fixes means nothing to re-audit). When set, a wall-clock timeout of 15 minutes applies across the entire convergence loop.
...
- **All other lead-run phases** (context gathering, dedup, validation, cleanup): Default model is fine — these are mechanical steps. **Exception — Phase 6 is not lead-run work**: its regression-fix and post-advisor-retry dispatches (`protocols/fix-secret-validate.md`) are implementer spawns that write code, so they take `subagent_type: "jr-implementer"` and `model: "opus"` per the implementer bullet above. Left unpinned, the tier resolves by accident of the agent-type file's own frontmatter, and an untyped spawn would fall through to the sonnet lead. Any agent spawned in these phases also honors a `--model` override (the override is total, not premium-sites-only).
...
|---|---|
| `.claude/review-profile.json` | Stack detection cache. A poisoned cache could set `validationCommands` to all-null, silently disabling validation, or inject shell metacharacters that execute in Phase 6. |
...
| `.claude/secret-warnings.json` | Secret audit trail. Records file paths, line numbers, and pattern types where secret patterns were detected — committing exposes the locations of current or historical secrets. |
...
matching row in `shared/gitignore-enforcement.md`'s "Sites that apply this protocol" table. Reasons
are copies of that canonical's "Why" column — re-sync them whenever it changes.
...
After all reviewers complete (or time out), collect their findings from the results returned in their completion notifications. There is no task list to read: the lead has no `TaskList`, and no reviewer role has `TaskCreate` (`../shared/subagent-reporting.md`). As the lead:

0.0. **Reviewer roll-call** (canonical: `../shared/subagent-reporting.md` "Lead-side: reviewer roll-call"): reconcile the Phase 2 spawn list against the results actually returned. A reviewer that returned nothing, an empty result, or an error is `UNREPORTED` — a failure, never a clean dimension. Append every `UNREPORTED` member per that file's accumulation rule: always to the run-level `unreported` set (Phase 7 reads `unreportedCount` and the names from it), and — when a loop is running it, i.e. under `--converge` — also to that loop's `passUnreported`. This runs FIRST because every later step is computed over the delivered set. It is **numbered** so `convergence-protocol.md`'s by-number step inheritance carries it. The health-score and `health.json` consequences are enforced where they occur (Phase 7 "Save health snapshot" incomplete-run gate, and `protocols/phase7-report.md` item 4) — this step's only job is to compute the state.
0. **Sanity-check findings (reject hallucinations)**: First recompute the Phase 2 freeze anchor; if it differs from the spawn value, the reviewed tree moved mid-pass — warn (`[FREEZE BROKEN — reviewed tree moved during pass]`) and flag this pass tree-moved (`../shared/audit-history-schema.md` "Skip stats-exempt rejections when the reviewed tree moved during a pass"). Then, before dedup, iterate every finding and verify its citation is real. Each reviewer was required to submit — per finding — a `file`, a `line`, and a `codeExcerpt` (3 consecutive lines from the cited file starting at `line`, verbatim with original whitespace; mandated in the reviewer prompt assembled in Phase 2). Run **all checks in parallel** via Bash — batch all file existence tests and line-count queries into a single multi-call message: `test -f "$file" && wc -l < "$file"` per finding. Reject a finding (drop it from the collected set) when (a) the `file` does not exist relative to the scope root, (b) `line` is not a positive integer, (c) `line` exceeds the file's line count, or (d) **content-excerpt mismatch** — read `file` lines `[line, line+2]` via the Read tool. **Dedupe by file before reading**: many findings cluster in the same file, so build a set of unique `(file, min-line, max-line)` tuples first, fetch each unique file once (batching all unique reads into a single message in parallel with the Bash checks), cache the content, then derive each finding's excerpt range from the cached content. Compare against the reviewer's `codeExcerpt` after normalizing both sides (strip trailing whitespace per line; collapse any run of blank lines to a single blank; treat tabs and spaces as equivalent when the only difference is indentation). If no line in `[line, line+2]` matches any line in the excerpt after normalization, reject the finding. **Output-scanning tolerance:** the harness scans a subagent's report before it reaches the lead and may rewrite it, inserting a backslash to neutralize instruction-shaped text — control tags (`<system-reminder>` and the other harness envelope tags), `antml:` model-layer tags, a forged `[harness:` line prefix, and `Human:`/`Assistant:` turn markers. Some of those categories also prepend a `[harness: subagent output matched instruction-shaped pattern(s): …]` marker line, but **turn markers are neutralized silently, with no marker emitted at all** — so never condition the un-escape on having seen a marker. Before comparing, ignore any such leading marker line and strip a harness-inserted backslash from instruction-shaped lines; never reject as `excerpt-mismatch` solely because the harness escaped one — the file side is read fresh and unscanned, so a genuine quote still matches once the escape is removed (claim stamped + re-verification method: `../jr-review/protocols/finding-sanity-check.md`). If the excerpt is missing or empty on a finding, treat it as hallucination evidence and reject. Log each rejection under `[REJECTED — INVALID CITATION]` with the reviewer's dimension name, the cited `file:line`, the reason (one of: missing-file / bad-line / line-out-of-range / excerpt-missing / excerpt-mismatch), and — for excerpt-mismatch rejections — a 2-line diff showing what the reviewer claimed vs. what the file actually contains; on a tree-moved pass, tag `excerpt-mismatch` rejections `statsExempt` (still dropped, but excluded from the rejection rate and from the Save-audit-history stats). Include all rejections in the Phase 7 report. Track the rejection rate per reviewer dimension over assessable findings (`statsExempt` rejections and their findings excluded from both numerator and denominator); if a single reviewer exceeds 25% rejection, emit a Phase 7 `ACTION REQUIRED` note so the user can investigate whether that reviewer hit its turn limit or was confused by the file set. The content-excerpt check catches a subtler hallucination than the line-range check alone: real line number + fabricated problem description. If the reviewer couldn't quote the line, they probably couldn't read the line.
0.5. **Verify claims (reject + cap hallucinated external facts)**: Apply `../shared/claim-verification.md`. For every finding that survived step 0, the **lead** (not the reviewer) classifies it by scanning its title/description for external-authority language (`deprecated`, `removed in`, `as of version`, `violates the rules of`, `OWASP`, `CVE-`, `WCAG`, `best practice`, version numbers tied to a behavior claim), defaulting to **external-authority when in doubt**; a finding is code-internal only when its correctness is provable from the cited excerpt plus other local files (already covered by step 0). For each external-authority claim, first try to ground it in the project's own internal official documentation (pinned dependency version in `package.json`/lockfile, local `.d.ts`/type defs, config) — if grounded, keep it and tag the grounding source. Otherwise, **by default fetch an authoritative source** (skip only under `--no-verify-claims`) per the doctrine's source-fetch discipline (`gh api …/contents/<path>` raw markdown, or the doc's raw `.md`; never rest a load-bearing claim on a single `WebFetch` summary — cross-check it). This `gh api` fetches an *external authority* (docs), so it stays `gh` even when the user's repo is on GitLab — external-authority carve-out, `../shared/forge-detection.md` §e; only user-repo forge ops switch with the detected forge. `confirmed` → keep and tag `[verified: <source> <date>]`; `refuted` → drop the finding, logged under `[REJECTED — CLAIM REFUTED BY SOURCE]` with the cited `file:line`, the claim, and the contradicting source; `unreachable`/`ambiguous`/`--no-verify-claims` → fall through to the cap. **In the fall-through (verification skipped, unreachable, or uncorroborable)**, cap the finding's confidence to `speculative` and tag it `[unverified external claim]` (its original severity is preserved for display — see step 3). Verify each unique claim-source at most once per run (in-memory dedup; no persisted cache in v1). Track a per-reviewer `refuted` rate; if a single dimension exceeds 25% refuted, emit a Phase 7 `ACTION REQUIRED` note (mirrors step 0's escalation).
...
5. **Assign file ownership**: Group findings by file for implementer dispatch.

**Display**: Compact summary: `28 raw → 18 deduplicated (5 merged, 5 dropped), 3 root-cause clusters`. Update timeline.

**Zero findings**: If no findings remain after dedup **AND `unreportedCount == 0`**, skip Phases 4-6. Output: "Clean audit — no findings." Proceed directly to Phase 7 for cleanup and report. (canonical: ../shared/subagent-reporting.md "Lead-side: reviewer roll-call")

**Zero findings with `unreportedCount > 0`**: the run is **not** clean — it is incomplete. Never emit the "Clean audit" string, never emit a health score computed over the delivered subset as if it covered the scope, skip to Phase 7, render the unreported dimensions by name per step 0.0, and exit non-zero.
...
- Include the **Subagent-facing block** of `../shared/subagent-reporting.md` (read into lead context at Phase 1 Track A) verbatim in each implementer's prompt, and state what its final response must carry: a per-finding verdict of `addressed` or `contested` (with the one-line reason). Fix-verification and the Phase 7 report are both computed from that response.
...
**Skip if `nofix` is set.** Apply the Phase 5.6 (secret re-scan) and Phase 6 (validate-fix loop) bodies from `${CLAUDE_SKILL_DIR}/protocols/fix-secret-validate.md` (loaded at Phase 5 entry) verbatim, for both phases.

## Phase 6 — Validate-fix loop

Body in `protocols/fix-secret-validate.md` (loaded at Phase 5 entry; see the Phase 5.6 pointer). Apply that protocol verbatim.
...
**Declare-done advisor check (gated)**: Before rendering the report, call `advisor()` once (no parameters — the full transcript is auto-forwarded) **if** the run is non-trivial: `findingCount >= 5 OR dimensionCount >= 3 OR rejectionCount >= 1 OR any fix was applied in Phase 5/6 OR abortMode=true`. Per `../shared/advisor-criteria.md`, declaring done on multi-phase work warrants a second opinion; the gate skips trivially-clean small runs (the "Unconditional advisor on every run" anti-pattern). **Make the deliverable durable BEFORE this call** (`../shared/advisor-criteria.md` criterion 2). A Phase 5/6 fix is already in the working tree, but when no fix was applied — `nofix`, or a run where every finding was declined — nothing is persisted yet and the report file is written below (step "write the audit report file"), *after* this call, so an interruption here would lose the entire finding set. On any run with no applied fix, do the report-file write and the `audit-history.json` append FIRST, then call the advisor, then append any `ADVISOR NOTES` to the already-written file. If the advisor concurs, proceed silently. If it raises a concrete concern (e.g., a fix that doesn't match its finding, a missed regression, a contested finding that should have blocked), surface it in the report under an `ADVISOR NOTES` line. The Phase 5 pre-dispatch advisor covers the substantive-edit boundary; this one covers the run as a whole.
...
The rendered marker appears at the top of the Phase 7 report on its own line so the run outcome is unmistakable. The full set of exit-forcing **markers** is defined in `../shared/abort-markers.md` ("Exit-code contribution") — consult that file as the single source of truth **for markers** (the set includes `[AUDIT-HISTORY BACKUP FAILED]` in addition to the abort/revert/secret/fresh-eyes/audit-trail/secret-warnings markers). It is **not** the whole exit-code rule: some conditions force a non-zero exit without emitting a marker, and `abort-markers.md` says so itself. The consolidated list lives in `${CLAUDE_SKILL_DIR}/protocols/phase7-report.md` ("Exit-code rules") — `unreportedCount > 0` is one of the non-marker conditions, so a lead that consults only the marker table would exit 0 on a lost dimension.
