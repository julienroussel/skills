...
- `--converge[=N]` — Re-audit modified files after Phase 6 (validate-fix loop) until no new findings emerge OR `N` iterations are reached. **Default N depends on `CLAUDE_EFFORT`** (read at runtime via Bash: `effort="$CLAUDE_EFFORT"; [ -z "$effort" ] && effort=high`): `low`/`medium` → 2, `high` → 3, `xhigh`/`max` → 5 (mirrors `/jr-review`'s effort-adaptive default, bounded by this skill's cap below). This skill declares `effort: max`, so bare `--converge` resolves to 5 on Claude Code ≥ 2.1.133. An explicit `--converge=N` overrides the effort-adaptive default. Capped at `[2, 5]` — values outside the range warn and are clamped (`N=1` is upgraded to 2 with a warning since one iteration provides no re-audit; `N>5` is clamped to 5 to limit runtime). Mutually exclusive with `nofix` (no fixes means nothing to re-audit). When set, a wall-clock timeout of 15 minutes applies across the entire convergence loop.
