...
**Re-arm the freeze anchor per pass** — capture a fresh `FREEZE_ANCHOR` at this pass's Phase 2 spawn and re-check it at this pass's Phase 3 step 0 (`protocols/phase2-reviewers.md` "Freeze the reviewed tree during the pass"). The between-pass Phase 5/5.5/6 edits legitimately move the tree, but they fall *outside* any pass's spawn→step-0 window, so they never trip the check — each pass reviews a settled tree. Each `excerpt-mismatch` rejection is tagged `statsExempt` (or not) at its own pass's step 0 and travels with the rejection record, so the per-pass `reviewerStats` write below (the `iteration` entries) exempts each pass by its own tags — no shared flag to reset.
...
2. **Re-run Phases 2 → 6** in sequence, with these constraints:
   ...
   - **Phase 3 step 0**: codeExcerpt content check still applies (working-tree mode — `/jr-audit` has no `--pr` mode), including the freeze-anchor re-check — a tree-moved convergence pass tags its `excerpt-mismatch` rejections `statsExempt`.
   ...
   - **Phase 4 approval**: the user re-approves each pass's findings interactively. The pre-approval advisor check (skewed-dimension / high-volume signals): **SKIP** — it already ran on the initial pass, and `SKILL.md` scopes it to at most once per audit run. The pre-iteration advisor check (#5 above) covers convergence-specific concerns, exactly as it does for the Phase 5 pre-dispatch check below.
   - **Phase 4.5**: append to `.claude/audit-history.json` with the same `runId` as the initial pass (a single `/jr-audit` invocation produces one logical history entry; convergence iterations are sub-events). Add an `iteration` field to `runs[]` and `reviewerStats[]` entries written during convergence so per-pass FP rates are visible. Exclude this pass's `statsExempt` rejections from those entries, exactly as the initial-pass write does (`../shared/audit-history-schema.md`).
